<template>
  <div class="page-container page-store-edit">
...
import { updateStore, getStoreById, getBrandServices} from '@/api/store'
...
export default {
  ...
  methods:{
    ...
    doUpdateStore() {
      ...
      const id = this.id
      const data = Object.assign({id: id}, this.form)
      updateStore(id,  data)
        .then(res => {
          if (res.success && res.entity) {
            ...
            this.$message.error(res.message || '修改网点信息失败')
          }
        })
        .catch(err => {
          this.$message.error(err.message || '修改网点信息失败！')
        })
        .finally(() => {
