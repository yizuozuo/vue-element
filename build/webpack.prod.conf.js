...
const isPrdConf = process.env.env_config === 'prd'
  ...
  plugins: [
    ...
    new webpack.optimize.CommonsChunkPlugin({
      ...
      minChunks (module) {
        ...
        const matchContext = context ? context.split(path.sep).join('::') : ''
        const targets = ['vue', 'vue-router', 'vuex']
        // /^(vue|vue-router)$/i
        const npmRegExp = new RegExp(targets.join('|'), 'i')
        // /^(_vue@2.4.2@vue|_vue-router@1.2.3@vue-router)$/i
        const cnpmRegExp = new RegExp(targets.map(t => `_${t}@\\d\\.\\d\\.\\d@${t}`).join('|'), 'i')
